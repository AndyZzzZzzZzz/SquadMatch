...
1. Databse schema is automatically generated by rails after running migrations. Migrations are ruby script that allow modifying database schema overtime in a consistence way. Each migration represents a single change to database schema, such as creating a table, adding a column, etc.
2. Models and Relationships define how the ruby classes is mapped to database table and how they relate to each other. Models are defined in app/models/ and corresponds to database tables. The purpose of model is to encapsulate the data, define associations, validations, and business logic.
3. ActiveRecord is the Object-Relational Mapping (ORM) layer in Rails. It allows the interactions between database tables as if they were Ruby classes. It provides methods for CRUD (Create, Read, Update, and Delete) operations, and simplifies handling relationships between tables/models.
